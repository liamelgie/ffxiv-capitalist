...
const ListingTableCondensed = (props) => {
    ...
    if (isLoading) return (
        <ListingTablePlaceholder limit={limit} crossWorld={crossWorld} condensed={true} />
    )
    ...
    const rows = listings.slice(0, limit).map((listing, i) => {
        const dateObject = new Date(listing.lastReviewTime * 1000)
        return (
            <tr key={i}>
